...
class UserDetail(APIView): #diff url so create diff clASS for this 
    ...
    def get_user(self,employee_id):   #search criteria for data 
        try:
    ...
    def get(self,request,employee_id): #methods
        if not self.get_user(employee_id):
           return  Response(f'user with(employee_id)is not found in db',status=status.HTTP_404_NOT_FOUND)
        serializer = UserSerializer(self.get_user(employee_id))
        return Response(serializer.data)
   
    def put(self,request,employee_id):   #to manipulate data
